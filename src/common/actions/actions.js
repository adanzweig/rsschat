...
export function receiveMemberRawChannel(channel) {
  return {
    type: types.RECEIVE_MEMBER_CHANNEL,
    channel
  };
}
...
function delChannel(json) {
  return {
    type: types.REMOVE_CHANNEL,
    json
  };
}
function requestDelChannel(json){
   return {
    type: types.REQ_DEL_CHANNEL,
    json
  }
}
...
export function joinChannel(user,channel) {
  return dispatch => {
    dispatch(joinChannelfx())
    return fetch(`/api/channels/join/${user}/${channel}`)
      .then(response => response.json())
      .then(json => dispatch(receiveChannels(json)))
      .catch(error => {throw error});
  }
...
export function fetchAllChannels() {
  return dispatch => {
    ...
    return fetch(`/api/channels`)
      .then(response => response.json())
      .then(json => dispatch(receiveChannelsAll(json)))
      .catch(error => {throw error});
  }
}
...
function requestAllChannels() {
  return {
    type: types.LOAD_ALL_CHANNELS
  }
}

function receiveChannelsAll(json) {
  return {
    type: types.LOAD_ALL_CHANNELS_SUCCESS,
    json
  }
}
...
export function deleteChannel(channel) {
  return dispatch => {
    dispatch(requestDelChannel())
    return fetch(`/api/channels/delete/${channel}`)
      .then(response => response.json())
      .then(json => dispatch(delChannel(json, channel)))
      .catch(error => {throw error});
    }
}
